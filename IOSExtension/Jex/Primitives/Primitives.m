...
- (void)cleanScreenInContext:(EAGLContext *)context {
	[self cleanRect:CGRectMake(0, 0, 1024, 768) inContext:context withColor:kColor4FWhite];
}

- (void)cleanScreenInContext:(EAGLContext *)context withColor:(kColor4F)color {
	[self cleanRect:CGRectMake(0, 0, 1024, 768) inContext:context withColor:color];
}
...
- (void)drawPoint:(CGPoint)point inContext:(EAGLContext *)context withColor:(kColor4F)color width:(GLfloat)width {
	GLfloat vertices[] = {
		point.x,	point.y,	DEFAULT_DEPTH
	};
...
- (void)drawPoints:(CGPoint *)points pointAmount:(NSInteger)count 
		 ...
		 withColor:(kColor4F)color width:(GLfloat)width {
	
	if (count < 1 || color.a == 0) {
...
- (void)drawLineFrom:(CGPoint)startPoint to:(CGPoint)endPoint 
	...
	GLfloat vertices[] = {
		startPoint.x,	startPoint.y,	DEFAULT_DEPTH,
		endPoint.x,		endPoint.y,		DEFAULT_DEPTH
	};
...
- (void)drawLines:(CGPoint *)points pointAmount:(NSInteger)count 
	...
	if (isSeg) {
		...
		for (int i = 0; i < count - 1; i++) {
			...
			vertices[6 * i + 3] = points[i + 1].x;
			vertices[6 * i + 4] = points[i + 1].y;
...
- (void)drawRect:(CGRect)rect inContext:(EAGLContext *)context withColor:(kColor4F)color width:(CGFloat)width {
	if (CGRectEqualToRect(rect, CGRectZero) || CGRectEqualToRect(rect, CGRectNull) || color.a == 0) {
		return;
	}
	GLfloat vertices[] = {
		rect.origin.x,						rect.origin.y,						DEFAULT_DEPTH,		
		rect.origin.x,						rect.origin.y + rect.size.height,	DEFAULT_DEPTH,		
		rect.origin.x + rect.size.width,	rect.origin.y + rect.size.height,	DEFAULT_DEPTH,		
		rect.origin.x + rect.size.width,	rect.origin.y,						DEFAULT_DEPTH
	};
...
- (void)fillRect:(CGRect)rect inContext:(EAGLContext *)context withColor:(kColor4F)color gradient:(const GLfloat *)glColors {
	if (CGRectEqualToRect(rect, CGRectZero) || CGRectEqualToRect(rect, CGRectNull) || (color.a == 0 && !glColors)) {
		return;
	}
	GLfloat vertices[] = {
		rect.origin.x,						rect.origin.y,						DEFAULT_DEPTH,		
...
- (void)drawRectsInContext:(EAGLContext *)context withColor:(kColor4F)color width:(CGFloat)width 
...
- (void)fillRectsInContext:(EAGLContext *)context withColor:(kColor4F)color gradient:(const GLfloat *)glColors
					sRects:(NSString *)simgRect, ... {
...
- (void)fillHollowRect:(CGRect)rect inContext:(EAGLContext *)context withColor:(kColor4F)color gradient:(const GLfloat *)glColors {
	...
	GLfloat vertices[] = {
		...
		0,                                  0,                                  DEFAULT_DEPTH,
		rect.origin.x,						rect.origin.y + rect.size.height,	DEFAULT_DEPTH,
		0,                                  1500,                               DEFAULT_DEPTH,
		rect.origin.x + rect.size.width,	rect.origin.y + rect.size.height,	DEFAULT_DEPTH,	
        1500,                               1500,                               DEFAULT_DEPTH,
		rect.origin.x + rect.size.width,	rect.origin.y,						DEFAULT_DEPTH,
        1500,                               0,                                  DEFAULT_DEPTH,
        rect.origin.x,						rect.origin.y,						DEFAULT_DEPTH,
		0,                                  0,                                  DEFAULT_DEPTH
	};
	
	GL_DRAW_ARRAY(context, color, glColors, DEFAULT_LINE_WIDTH, vertices, 0, 10, GL_TRIANGLE_STRIP);
}
...
- (void)drawEllipse:(CGRect)rect inContext:(EAGLContext *)context withColor:(kColor4F)color width:(CGFloat)width {
	...
	for (int i = 0; i < 360; i++) {
		vertices[3 * i + 0] = xMid + xRadius * cos(DEGREES_TO_RADIANS(i));
		vertices[3 * i + 1] = yMid + yRadius * sin(DEGREES_TO_RADIANS(i));
		vertices[3 * i + 2] = DEFAULT_DEPTH;
	}
...
- (void)drawEllipsesInContext:(EAGLContext *)context withColor:(kColor4F)color width:(CGFloat)width 
					   sRects:(NSString *)simgRect, ... {
...
- (void)fillEllipsesInContext:(EAGLContext *)context withColor:(kColor4F)color gradient:(const GLfloat *)glColors 
					   sRects:(NSString *)simgRect, ... {
...
- (void)fillHollowEllipse:(CGRect)rect inContext:(EAGLContext *)context withColor:(kColor4F)color gradient:(const GLfloat *)glColors {
    CGFloat xRadius = rect.size.width / 2,	yRadius = rect.size.height / 2;
	CGFloat xMid = CGRectGetMidX(rect),		yMid = CGRectGetMidY(rect);
	GLfloat vertices[3 * 360];
	for (int i = 0; i < 360; i++) {
...
- (void)drawArcAt:(CGPoint)center radius:(CGFloat)radius 
	...
	NSInteger count = (NSInteger)(endAngle - startAngle + 720) % 360;
	GLfloat vertices[3 * count];
	for (int i = 0; i < count - 1; i++) {
		vertices[3 * i + 0] = center.x + radius * cos(DEGREES_TO_RADIANS(i + startAngle));
		vertices[3 * i + 1] = center.y + radius * sin(DEGREES_TO_RADIANS(i + startAngle));
		vertices[3 * i + 2] = DEFAULT_DEPTH;
	}
...
- (void)drawBezierCurve:(kBezier)bPoints inContext:(EAGLContext *)context withColor:(kColor4F)color width:(CGFloat)width {
	// TODO: implement late
	if (CGPointEqualToPoint(bPoints.sp, bPoints.ep)) {
		...
		if (CGPointEqualToPoint(bPoints.cp1, bPoints.cp2)) {
			for (int i = 0; i < segments; i++) {				
				vertices[i * 3] = powf(1 - t, 2) * bPoints.sp.x + 2.0f * (1 - t) * t * bPoints.cp1.x + t * t * bPoints.ep.x;
				vertices[i * 3 + 1] = powf(1 - t, 2) * bPoints.sp.y + 2.0f * (1 - t) * t * bPoints.cp1.y + t * t * bPoints.ep.y;
				vertices[i * 3 + 2] = DEFAULT_DEPTH;
				t += 1.0f / segments;
			}			
		} else {
			for (int i = 0; i < segments; i++) {
				vertices[i * 3] = powf(1 - t, 3) * bPoints.sp.x 
											 + 3.0f * powf(1 - t, 2) * t * bPoints.cp1.x
											 + 3.0f * (1 - t) * t * t * bPoints.cp2.x 
											 + t * t * t * bPoints.ep.x;
				vertices[i * 3 + 1] = powf(1 - t, 3) * bPoints.sp.y 
...
- (void)drawRoundRect:(CGRect)rect radius:(CGFloat)radius 
	...
	for (int i = 0; i < 360; i++) {
		...
		vertices[3 * i + 0] = radius + radius * cos(DEGREES_TO_RADIANS(i)) + xOffset;
		vertices[3 * i + 1] = radius + radius * sin(DEGREES_TO_RADIANS(i)) + yOffset;
		vertices[3 * i + 2] = DEFAULT_DEPTH;		
	}
...
- (void)fillStar:(NSInteger)pointAmount at:(CGPoint)center radius:(CGFloat)radius angle:(CGFloat)angle 
	...
	for (int i = 0; i < pointAmount; i++) {
		...
		if (i == 0) {
			vertices[3 * pointAmount + 0] = x;
			vertices[3 * pointAmount + 1] = y;
			vertices[3 * pointAmount + 2] = DEFAULT_DEPTH;
		} else if (i == 1) {
			vertices[3 * pointAmount + 3] = x;
			vertices[3 * pointAmount + 4] = y;
...
- (void)drawFanAt:(CGPoint)center radius:(CGFloat)radius 
	...
	NSInteger count = (NSInteger)(endAngle - startAngle + 720) % 360;
	if (count == 0 && endAngle != startAngle) {
        count = 360;
    }
	GLfloat vertices[6 * count + 3];
	vertices[0] = center.x;
	vertices[1] = center.y;
	vertices[2] = DEFAULT_DEPTH;
	for (int i = 0; i < count; i++) {
		vertices[6 * i + 3] = center.x + radius * cos(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 4] = center.y + radius * sin(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 5] = DEFAULT_DEPTH;
		vertices[6 * i + 6] = center.x + radius * cos(DEGREES_TO_RADIANS(i + startAngle + 1));
		vertices[6 * i + 7] = center.y + radius * sin(DEGREES_TO_RADIANS(i + startAngle + 1));
		vertices[6 * i + 8] = DEFAULT_DEPTH;
	}
	
	GL_DRAW_ARRAY(context, color, nil, width, vertices, 0, 2 * count + 1, GL_LINE_LOOP);
}
...
- (void)fillFanAt:(CGPoint)center radius:(CGFloat)radius 
	...
	GL_DRAW_ARRAY(context, color, glColors, DEFAULT_LINE_WIDTH, vertices, 0, 2 * count + 1, GL_TRIANGLE_FAN);
}
...
- (void)fillHollowFanAt:(CGPoint)center radius:(CGFloat)radius 
	...
	GLfloat vertices[6 * (count + 1)];
	for (int i = 0; i < count; i++) {
		vertices[6 * i + 0] = center.x + radius * cos(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 1] = center.y + radius * sin(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 2] = DEFAULT_DEPTH;
		vertices[6 * i + 3] = center.x + 1500 * cos(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 4] = center.y + 1500 * sin(DEGREES_TO_RADIANS(i + startAngle));
		vertices[6 * i + 5] = DEFAULT_DEPTH;
	}
    vertices[6 * count + 0] = center.x + radius * cos(DEGREES_TO_RADIANS(startAngle));
    vertices[6 * count + 1] = center.y + radius * sin(DEGREES_TO_RADIANS(startAngle));
    vertices[6 * count + 2] = DEFAULT_DEPTH;
    vertices[6 * count + 3] = center.x + 1500 * cos(DEGREES_TO_RADIANS(startAngle));
    vertices[6 * count + 4] = center.y + 1500 * sin(DEGREES_TO_RADIANS(startAngle));
    vertices[6 * count + 5] = DEFAULT_DEPTH;
	
	GL_DRAW_ARRAY(context, color, glColors, DEFAULT_LINE_WIDTH, vertices, 0, 2 * (count + 1), GL_TRIANGLE_STRIP);
}
...
- (void)drawTexture:(GLuint)texture at:(CGPoint)center 
	...
	glPushMatrix();
	{
		...
		if ((NSInteger)startAngle % 45 != 0) {
			...
			if (VALUE_BETWEEN_OO(startAngle, 45, 135) || VALUE_BETWEEN_OO(startAngle, 225, 325)) {
				xStart = 1 - (int)startAngle / 180;
				float tanValue = tan(DEGREES_TO_RADIANS(startAngle));
				yStart = 0.5 - (xStart - 0.5) / tanValue;
			} else {
				yStart = VALUE_BETWEEN_OO(startAngle, 90, 270) ? 1 : 0;
				float tanValue = tan(DEGREES_TO_RADIANS(startAngle));
			...
			vertices[2 * verticeCount] = xStart;
			texcoords[2 * verticeCount + 1] = yStart;
			vertices[2 * verticeCount + 1] = yStart;
